...
sed -i 's/DROPBEAR_EXTRA_ARGS=/DROPBEAR_EXTRA_ARGS="-p 222 -p 110 -p 109"/g' /etc/default/dropbear
...
echo "Dropbear : 445, 110, 109"  | tee -a log-install.txt
echo "Squid3    : 443, 80, 8080 (limit to IP SSH)"  | tee -a log-install.txt
